...
from App.models import MainWheel, MainNav, MainMustBuy, MainShop, MainShow, FoodType, Goods, AXFUser, Cart, Order, \
    OrderGoods
from App.views_constant import ALL_TYPE, ORDER_TITLE, ORDER_PRICE_UP, ORDER_PRICE_DOWN, ORDER_SALE_UP, ORDER_SALE_DOWN, \
    HTTP_USER_EXIST, HTTP_OK, HTTP_EMAIL_EXIST, ORDER_STATUS_NOT_PAY, ORDER_STATUS_NOT_RECEIVE, ORDER_STATUS_NOT_SEND
from App.views_helper import hash_str, send_email_activate, get_total_price
from GPAXF.settings import MEDIR_KEY_PREFIX
...
def market_with_params(requset, typeid, childcid, order_rule):
    ...
    return render(requset, 'main/market.html', context=data)
...
def mine(request):
    ...
    if user_id:
        ...
        data["icon"] = MEDIR_KEY_PREFIX + user.u_icon.url
...
def check_user(request):
    ...
    u_email = AXFUser.objects.filter(u_email=email)
